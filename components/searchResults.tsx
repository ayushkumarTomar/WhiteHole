import { View, Text, StyleSheet, ScrollView, FlatList, Image, TouchableOpacity, ToastAndroid } from 'react-native'
...
const SearchResults = ({ continueListeningData, title }: any) => {
   ...
   const handleContextMenu = async(value:any , songId:string)=>{
    ...
    const trendingItem = ({ item }: any) => {
        ...
        const imageLink = item.image.replace(/-(\d+)x(\d+)(?=\.\w+($|\?))/, "-500x500");
