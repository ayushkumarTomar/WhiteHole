import { View, Text, StyleSheet, ScrollView, FlatList, Image, ToastAndroid, TouchableOpacity } from 'react-native'
...
const TrendingSearch = ({ continueListeningData , title }: any) => {
   ...
   const handleContextMenu = async(value:any , songId:string)=>{
    ...
    const trendingItem = ({ item }: any) => {
        ...
        const imageLink = item.image.replace(/-(\d{3})x(\d{3})(?=\.\w+($|\?))/, "-500x500");
...
const styles = StyleSheet.create({
    continue: {
        marginTop: vs(10), // Add margin if needed between header and this section
        paddingLeft: s(10),
    },
    continueText: {
